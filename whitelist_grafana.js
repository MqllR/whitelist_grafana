...
const main = async () => {
  try {
    ...
    if (!ipset_list.stdout.includes(ipset_whitelist)) {
      await exec(
        `ipset create ${ipset_whitelist} hash:ip family inet hashsize 1024`
      );
  ...
  const iptables_rules = `DOCKER-USER -p tcp -m tcp --dport ${dest_port} \
	-m set ! --match-set ${ipset_whitelist} src -j DROP`;

  try {
    await exec(`iptables -C ${iptables_rules}`);
